...
# input files
# primitive cell file
primitive_cell="${INPUT_DIR}/${input_filename}"
# relax input
...
# working files
# relaxed file
relaxed_file="${WORKING_DIR}/relaxed.xyz"
# forces file
...
echo '[INFO] Relaxing primitive cell'
# relax the cell, output to single_relaxed.xyz
"${lammps}" -in ${relax_input} -var WORKING_DIR ${WORKING_DIR} -var OUTPUT_RELAXED "${relaxed_file}" -var INPUT_DATA ${primitive_cell} -var MODEL_FILE ${model_file}
mv *.xyz ${WORKING_DIR}

echo '[INFO] generating supercell with phonopy'
# generate supercell with phonopy
"${phonopy}" --lammps -c "${relaxed_file}" -d --dim "${n_replica_x} ${n_replica_y} ${n_replica_z}"
